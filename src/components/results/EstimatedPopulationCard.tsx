...
export function EstimatedPopulationCard({ results }: { results: ResultsData }) {
  const { poblacional } = results;

  return (
    <Card variant={poblacional.disponible ? 'green' : 'default'}>
      ...
      {poblacional.disponible ? (
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-green-700">{poblacional.tuberculosTercera.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Tubérculos de Cuarta</span>
            <span className="font-bold text-blue-700">{poblacional.tuberculosCuarta.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center border-t pt-2 mt-1">
            <span className="text-sm font-semibold text-gray-700">Total Tubérculos</span>
            <span className="text-xl font-bold text-green-800">{poblacional.totalTuberculos.toLocaleString()}</span>
          </div>
        </div>
      ) : (
